...
import { AppBar, Toolbar, IconButton, Typography, Badge, Menu, MenuItem, Box, Divider } from '@mui/material';
...
const CustomNavBar = () => {
  ...
  const renderNotificationMenu = (
    <Menu
      ...
      onClose={handleMenuClose}
    >
      <MenuItem onClick={fetchNotifications}>Fetch Notifications</MenuItem>
      <Divider />
        ...
        notifications.map((notification, index) => (
          <MenuItem key={index} onClick={() => handleNotificationClick(notification)}>
            <Box>
              ...
              {notification.link ? (
                <Typography variant="body2" color="primary">Go to Link</Typography>
              ) : (
